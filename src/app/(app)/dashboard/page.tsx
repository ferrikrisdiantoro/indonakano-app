  ...
  const cards = [
    {
      ...
      value: pendingCount,
      icon: Clock,
      iconBg: pendingCount > 0 ? "bg-orange-100 text-orange-600" : "bg-slate-100 text-slate-500",
      accent: pendingCount > 0 ? "ring-orange-100" : "",
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {cards.map((card) => (
          <a
            key={card.label}
            href={card.href}
            className="group bg-white rounded-xl border border-slate-200 p-4 sm:p-5 hover:border-blue-200 hover:shadow-sm transition-all"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11px] sm:text-xs text-slate-500 font-medium uppercase tracking-wide">{card.label}</p>
                <p className="text-2xl sm:text-3xl font-bold text-slate-900 mt-1.5 group-hover:text-blue-700 transition-colors">{card.value}</p>
              </div>
